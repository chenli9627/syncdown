...
export function EditorAiChatThreadSwitcher({
  ...
  return (
    <div className="relative min-w-0" ref={switcherRef}>
      <button
        aria-expanded={open}
        className="flex h-8 max-w-52 min-w-0 items-center gap-2 border border-transparent px-1 text-left text-sm font-semibold text-[var(--color-foreground)] hover:bg-[var(--color-muted)]"
      ...
      {open ? (
        ...
        >
          ...
          {threads.map((thread) => {
            ...
            return (
              <button
                className="flex h-8 w-full items-center gap-2 px-2 text-left text-xs text-[var(--color-muted-foreground)] hover:bg-[var(--color-muted)] hover:text-[var(--color-foreground)]"
                key={thread.id}
                onClick={() => {
                  setOpen(false);
              ...
              >
                <span className="w-3 shrink-0">
                  {thread.id === activeThreadId ? <Check aria-hidden="true" size={12} /> : null}
                </span>
